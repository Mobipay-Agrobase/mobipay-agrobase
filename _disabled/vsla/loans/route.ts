...
import { calculateLoan, calculateMaxLoan, Refs, postJournalEntry, VSLA_TRANSACTION_TYPES, VSLA_LOAN_STATUS, writeAuditLogV3 } from '@/lib/vsla-engine';
...
export async function POST(req: NextRequest) {
  ...
  let product = null;
  if (productId) {
    product = await db.vslaLoanProductV3.findUnique({ where: { id: productId } });
