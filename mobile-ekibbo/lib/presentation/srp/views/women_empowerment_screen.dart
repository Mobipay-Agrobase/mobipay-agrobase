...
class WomenEmpowermentScreen extends StatefulWidget {
  const WomenEmpowermentScreen(
      {super.key, required this.srp, required this.date});
  final SRPActionModel srp;
  final DateTime date;
  @override
  State<WomenEmpowermentScreen> createState() => _WomenEmpowermentScreenState();
}

class _WomenEmpowermentScreenState extends State<WomenEmpowermentScreen> {
  ...
  _getDataRemote() async {
    final res = await ApiProvider.instance.apiSRP.getWomenEmpowermentForm(
  ...
  _getData() async {
    final res =
        await rootBundle.loadString('assets/data/woment_empowerment.json');
  ...
  _onSave() async {
    ...
    final res =
        await ApiProvider.instance.apiSRP.submitWomenEmpowermentForm(data);
    if (res?.result == true) {
      Navigator.of(context).pop(true);
      DialogHelper.showToast(context,
          'SRP ${QuestionSectionType.womenEmpowerment.title} Created Successfully');
    }
  }
  ...
  Widget build(BuildContext context) {
    return SurveyView(
      title: QuestionSectionType.womenEmpowerment.title,
