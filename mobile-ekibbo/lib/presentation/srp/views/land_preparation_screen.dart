...
class LandPreparationScreen extends StatefulWidget {
  const LandPreparationScreen({
    super.key,
    required this.date,
    required this.srp,
  });
  ...
  final DateTime date;
  @override
  State<LandPreparationScreen> createState() => _LandPreparationScreenState();
}

class _LandPreparationScreenState extends State<LandPreparationScreen> {
  ...
  _getDataRemote() async {
    final res = await ApiProvider.instance.apiSRP.getLandPreparationForm(
  ...
  _getData() async {
    final res =
        await rootBundle.loadString('assets/data/land_preparation.json');
  ...
  _onSave() async {
    ...
    for (var e in _questions) {
      if (e.section == 'land_preparation_informations') {
        if (e.answer != null) {
    ...
    final res =
        await ApiProvider.instance.apiSRP.submitLandPreparationForm(data);
    if (res?.result == true) {
      Navigator.of(context).pop(true);
      DialogHelper.showToast(context,
          'SRP ${QuestionSectionType.landPreparation.title} Created Successfully');
    }
  }
  ...
  Widget build(BuildContext context) {
    return SurveyView(
      title: QuestionSectionType.landPreparation.title,
