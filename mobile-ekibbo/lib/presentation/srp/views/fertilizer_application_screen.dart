...
class FertilizerApplicationScreen extends StatefulWidget {
  const FertilizerApplicationScreen({
    super.key,
    required this.date,
    required this.srp,
  ...
  @override
  State<FertilizerApplicationScreen> createState() =>
      _FertilizerApplicationScreenState();
}

class _FertilizerApplicationScreenState
    extends State<FertilizerApplicationScreen> {
  ...
  _getData() async {
    _questions.clear();
    final res =
        await rootBundle.loadString('assets/data/fertilizer_application.json');
  ...
  _onSave() async {
    ...
    for (var e in _questions) {
      if (e.section == 'fertilizer_application_information') {
        if (e.answer != null) {
      ...
      } else if (e.section == 'fertilizer_details' &&
              map2['fertilizer_mixed'] == null ||
          map2['cost_of_the_fertilixer'] == null) {
        if (e.answer != null) {
    ...
    final res =
        await ApiProvider.instance.apiSRP.submitFetilizerApplicationForm(data);
    if (res?.result == true) {
      Navigator.of(context).pop(true);
      DialogHelper.showToast(context,
          'SRP ${QuestionSectionType.fertilizerApplication.title} Created Successfully');
    }
  }
  ...
  Widget build(BuildContext context) {
    return SurveyView(
      ...
      title: QuestionSectionType.fertilizerApplication.title,
