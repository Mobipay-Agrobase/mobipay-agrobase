...
class IntegratedPestManagmentScreen extends StatefulWidget {
  const IntegratedPestManagmentScreen({
    super.key,
    required this.srp,
    required this.date,
  });
  ...
  final DateTime date;
  @override
  State<IntegratedPestManagmentScreen> createState() =>
      _IntegratedPestManagmentScreenState();
}

class _IntegratedPestManagmentScreenState
    extends State<IntegratedPestManagmentScreen> {
  ...
  _getDataRemote() async {
    final res = await ApiProvider.instance.apiSRP
        .getIntegratePestManagementForm(
  ...
  _getData() async {
    final res = await rootBundle
        .loadString('assets/data/integrated_pest_management.json');
  ...
  _onSave() async {
    ...
    final res = await ApiProvider.instance.apiSRP
        .submitIntegratePestManagementForm(data);
    if (res?.result == true) {
      Navigator.of(context).pop(true);
      DialogHelper.showToast(context,
          'SRP ${QuestionSectionType.integratedPestManagement.title} Created Successfully');
    }
  }
  ...
  Widget build(BuildContext context) {
    return SurveyView(
      title: QuestionSectionType.integratedPestManagement.title,
