...
import { postJournalEntry, VSLA_TRANSACTION_TYPES, writeAuditLog } from '@/lib/vsla-engine';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const claim = await db.vslaSocialFundClaim.findUnique({
    where: { id },
  ...
  if (action === 'approve') {
    const updated = await db.vslaSocialFundClaim.update({
      where: { id },
