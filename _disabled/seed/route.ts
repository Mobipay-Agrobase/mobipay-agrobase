...
export async function POST(req: NextRequest) {
  ...
  await db.vslaJournalEntryV3.deleteMany();
  await db.vslaAccountV3.deleteMany();
  await db.vslaTransactionV3.deleteMany();
  await db.vslaAttendanceV3.deleteMany();
  await db.vslaMeetingV3.deleteMany();
  await db.vslaLoanRepaymentV3.deleteMany();
  await db.vslaLoanGuarantorV3.deleteMany();
  await db.vslaLoanV3.deleteMany();
  await db.vslaLoanProductV3.deleteMany();
  await db.vslaCycleV3.deleteMany();
  await db.vslaSavingWithdrawalV3.deleteMany();
  await db.vslaSavingV3.deleteMany();
  await db.vslaFineV3.deleteMany();
  await db.vslaSocialFundClaimV3.deleteMany();
  await db.vslaSocialFundContributionV3.deleteMany();
  await db.vslaOfficerRoleV3.deleteMany();
  await db.vslaMemberV3.deleteMany();
  await db.vslaGroupV3.deleteMany();
  await db.smsMessageLog.deleteMany();
  await db.ussdSession.deleteMany();
  await db.payment.deleteMany();
  await db.nssfFarmerContribution.deleteMany();
  await db.revenueSplit.deleteMany();
  await db.partnerSettlement.deleteMany();
  await db.kilimoPartner.deleteMany();
  ...
  const ktPartner = await db.kilimoPartner.create({
    data: {
      code: 'KT',
      name: 'Kilimo Trust',
  ...
  for (const gd of groupsData) {
    const g = await db.vslaGroupV3.create({
  ...
  for (const group of groups) {
    ...
    for (let i = 0; i < memberCount; i++) {
      ...
      const m = await db.vslaMemberV3.create({
        data: {
          groupId: group.id,
    ...
    if (groupMembers.length >= 3) {
      ...
      for (let i = 0; i < 3; i++) {
        await db.vslaOfficerRoleV3.create({
          data: {
            groupId: group.id,
  ...
  for (const group of groups) {
    ...
    for (const p of products) {
      await db.vslaLoanProductV3.create({
  ...
  for (const member of members) {
    ...
    for (let i = 0; i < contributionCount; i++) {
      ...
      const saving = await db.vslaSavingV3.create({
        data: {
  ...
  for (const group of groups) {
    ...
    for (let i = 0; i < meetingCount; i++) {
      ...
      const meeting = await db.vslaMeetingV3.create({
        data: {
          groupId: group.id,
          meetingNumber: i + 1,
          title: `Meeting #${i + 1}`,
      ...
      for (const m of present) {
        await db.vslaAttendanceV3.create({
          data: {
  ...
  for (const member of members) {
    ...
    for (let i = 0; i < contribCount; i++) {
      ...
      await db.vslaSocialFundContributionV3.create({
        data: {
          groupId: member.groupId,
          memberId: member.id,
          amount,
  ...
  for (const group of groups) {
    ...
    for (let i = 0; i < claimCount; i++) {
      ...
      const amount = 50000 + Math.floor(Math.random() * 200000);
      await db.vslaSocialFundClaimV3.create({
        data: {
          groupId: group.id,
          memberId: member.id,
          amount,
  ...
  for (let i = 0; i < 30; i++) {
    ...
    await db.nssfFarmerContribution.create({
      data: {
        tenantId: tenant.id,
        farmerName: farmer,
