...
export async function GET(req: NextRequest) {
  ...
  const fines = await db.vslaFineV3.findMany({
    where,
...
export async function POST(req: NextRequest) {
  ...
  const fine = await db.vslaFineV3.create({
    data: {
...
export async function PATCH(req: NextRequest) {
  ...
  const fine = await db.vslaFineV3.findUnique({ where: { id: fineId }, include: { group: true } });
  ...
  if (action === 'waive') {
    const updated = await db.vslaFineV3.update({
      where: { id: fineId },
      data: { status: 'WAIVED' },
    });
    return NextResponse.json({ fine: updated });
