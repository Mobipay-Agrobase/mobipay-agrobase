...
import { Refs, writeAuditLogV3 } from '@/lib/vsla-engine';
...
export async function GET(req: NextRequest) {
  ...
  const contributions = await db.nssfFarmerContribution.findMany({
    where,
    orderBy: { createdAt: 'desc' },
  });

  const total = await db.nssfFarmerContribution.aggregate({
...
export async function POST(req: NextRequest) {
  ...
  const contribution = await db.nssfFarmerContribution.create({
    data: {
      tenantId,
  ...
  await db.nssfFarmerContribution.update({
