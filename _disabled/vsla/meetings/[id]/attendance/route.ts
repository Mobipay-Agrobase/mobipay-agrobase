...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const totalMembers = await db.vslaMemberV3.count({ where: { groupId: (await db.vslaMeetingV3.findUnique({ where: { id } }))?.groupId, status: 'ACTIVE' } });

  const meeting = await db.vslaMeetingV3.update({
    where: { id },
    data: {
      status: 'CONCLUDED',
