...
import {
  ...
  Plus, Search, ChevronRight, Globe, Layers, UserCheck, Handshake
} from 'lucide-react'
...
export default function SettingsView() {
  ...
  const initialTab = activeSubTab && (isSuperAdmin || !adminOnlyTabs.has(activeSubTab))
    ? activeSubTab
    : defaultTab
    ...
    <div className="space-y-4">
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <TabsList className="flex flex-wrap h-auto gap-1">
          ...
          {isSuperAdmin && <TabsTrigger value="modules" className="gap-1.5"><Layers className="w-3.5 h-3.5" /> Modules</TabsTrigger>}
          {isSuperAdmin && <TabsTrigger value="system" className="gap-1.5"><Database className="w-3.5 h-3.5" /> System</TabsTrigger>}
        </TabsList>
        ...
        <TabsContent value="cooperatives" className="mt-4">
          <Suspense fallback={<div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-12 w-full rounded" />)}</div>}>
            <CooperativeManagement />
          </Suspense>
        </TabsContent>

        <TabsContent value="catalog-master" className="mt-4">
          <Suspense fallback={<div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-12 w-full rounded" />)}</div>}>
            <CatalogManager />
          </Suspense>
        </TabsContent>
