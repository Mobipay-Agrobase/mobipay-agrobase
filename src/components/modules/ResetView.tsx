        ...
        <Card><CardHeader><CardTitle className="text-sm">By Settlement</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {data.breakdowns?.bySettlement?.map((s: any) => (
              <div key={s.name} className="flex justify-between text-sm"><span>{s.name}</span><span className="font-bold">{s.count}</span></div>
        ...
        <Card><CardHeader><CardTitle className="text-sm">By Partner</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {data.breakdowns?.byPartner?.map((p: any) => (
              <div key={p.name} className="flex justify-between text-sm"><span>{p.name}</span><span className="font-bold">{p.count}</span></div>
            )) || <p className="text-sm text-muted-foreground">No data yet</p>}
          </CardContent>
        </Card>
      </div>
...
function BeneficiariesTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const res = await fetch(`/api/reset/beneficiaries?page=${page}&limit=20`)
          ...
          <TableBody>
            {data?.beneficiaries?.map((b: any) => (
              <TableRow key={b.id}>
                ...
                <TableCell className="text-xs">{b.enrolledBy}</TableCell>
            ...
            )) || <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">No beneficiaries yet</TableCell></TableRow>}
          </TableBody>
        </Table>
      </CardContent></Card>
...
function EnrollBeneficiaryDialog({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) { setResult(data) } else { toast.error(data.error || 'Failed') }
  ...
  return (
    <Dialog open onOpenChange={onClose}><DialogContent className="max-w-md">
      ...
      <div className="space-y-3">
        ...
        <div><Label>Enrolled By</Label><Select value={form.enrolledBy} onValueChange={v => setForm({ ...form, enrolledBy: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{PARTNERS.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select></div>
      </div>
      <DialogFooter>
...
function VouchersTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const res = await fetch('/api/reset/vouchers?limit=20')
    if (res.ok) setData(await res.json())
    setLoading(false)
  }, [])
          ...
          <TableBody>
            {data?.vouchers?.map((v: any) => (
              <TableRow key={v.id}>
                ...
                <TableCell className="text-xs">{v.issuedBy}</TableCell>
                <TableCell className="text-xs">{formatDate(v.expiryDate)}</TableCell>
                <TableCell><Badge className={v.status === 'ISSUED' ? 'bg-amber-100 text-amber-700' : v.status === 'REDEEMED' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}>{v.status}</Badge></TableCell>
              </TableRow>
            )) || <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No vouchers yet</TableCell></TableRow>}
          </TableBody>
        </Table>
      </CardContent></Card>
...
function IssueVoucherDialog({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) { toast.success('Voucher issued'); onSaved() } else { const d = await res.json(); toast.error(d.error || 'Failed') }
    } catch { toast.error('Network error') } finally { setSaving(false) }
  }
  ...
  return (
    <Dialog open onOpenChange={onClose}><DialogContent className="max-w-md">
      ...
      <div className="space-y-3">
        <div><Label>Beneficiary</Label><Select value={form.beneficiaryId} onValueChange={v => setForm({ ...form, beneficiaryId: v })}><SelectTrigger><SelectValue placeholder="Select..." /></SelectTrigger><SelectContent>{beneficiaries.map(b => <SelectItem key={b.id} value={b.id}>{b.fullName} ({b.beneficiaryId})</SelectItem>)}</SelectContent></Select></div>
        <div className="grid grid-cols-2 gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><Label>Issued By</Label><Select value={form.issuedBy} onValueChange={v => setForm({ ...form, issuedBy: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{PARTNERS.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select></div>
      ...
      <DialogFooter><DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose><Button onClick={handleSave} disabled={saving || !form.beneficiaryId || !form.amount}>{saving ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}Issue</Button></DialogFooter>
    </DialogContent></Dialog>
  )
}
...
function MerchantsTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const res = await fetch('/api/reset/merchants?limit=20')
    if (res.ok) setData(await res.json())
    setLoading(false)
  }, [])
  ...
  return (
    <div className="space-y-4">
      ...
      <Card><CardContent className="p-0">
        <Table>
          <TableHeader><TableRow>
            <TableHead className="text-xs">Code</TableHead><TableHead className="text-xs">Business</TableHead>
          ...
          <TableBody>
            {data?.merchants?.map((m: any) => (
              <TableRow key={m.id}>
                ...
                <TableCell className="text-xs">{m.businessType}</TableCell>
                <TableCell className="text-xs text-right"><MaskedAmount amount={m.payoutAmount} /></TableCell>
                <TableCell><Badge className={m.status === 'APPROVED' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}>{m.status}</Badge></TableCell>
              </TableRow>
            )) || <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">No merchants yet</TableCell></TableRow>}
          </TableBody>
        </Table>
      </CardContent></Card>
    </div>
...
function CashTab() {
  return (
    <Card><CardContent className="p-6 text-center">
      ...
      <p className="text-sm text-muted-foreground mt-1">Upload a CSV with beneficiary phones + amounts to send bulk MTN/Airtel MoMo payments.</p>
      <p className="text-sm text-muted-foreground mt-2">Feature ready — CSV upload + bulk disbursement API pending integration with MTN/Airtel merchant credentials.</p>
    </CardContent></Card>
  )
}
