...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [group, stats, accounts, savings, loans, meetings, transactions] = await Promise.all([
    ...
    getGroupStats(id),
    db.vslaAccountV3.findMany({ where: { groupId: id }, orderBy: { code: 'asc' } }),
    db.vslaSavingV3.findMany({
      where: { groupId: id },
      include: { member: { select: { fullName: true, memberId: true } } },
      orderBy: { createdAt: 'desc' },
      take: 100,
    }),
    db.vslaLoanV3.findMany({
      where: { groupId: id },
      include: {
    ...
    db.vslaMeetingV3.findMany({
      where: { groupId: id },
      orderBy: { meetingDate: 'desc' },
      take: 20,
      include: { _count: { select: { attendance: true } } },
    }),
    db.vslaTransactionV3.findMany({
      where: { groupId: id },
      orderBy: { createdAt: 'desc' },
      take: 100,
    }),
