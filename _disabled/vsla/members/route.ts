...
export async function GET(req: NextRequest) {
  ...
  const members = await db.vslaMemberV3.findMany({
    where,
    include: {
  ...
  const enriched = await Promise.all(
    members.map(async (m) => {
      const savings = await db.vslaSavingV3.aggregate({
        where: { memberId: m.id, status: 'COMPLETED' },
        _sum: { amount: true },
      });
      const shares = await db.vslaSavingV3.aggregate({
        where: { memberId: m.id, status: 'COMPLETED' },
        _sum: { sharesBought: true },
      });
      const outstandingLoans = await db.vslaLoanV3.aggregate({
...
export async function POST(req: NextRequest) {
  ...
  const member = await db.vslaMemberV3.create({
    data: {
      groupId,
      fullName,
      phone,
